...
function search(){
    ...
    let search;
    let artist, album, title, mbid, genre, length, rating, recordId;
        ...
        response.json().then(function(value){
            ...
            if (totalSearch == value.count) finish = true;
                ...
                value.recordings.map(function(record){
                        ...
                        if (select == "album"){
                            for (let i=0; i<record.releases.length; i++){
                                if (record.releases[i]['release-group'].title.toLowerCase().includes(input.toLowerCase())){
                                    console.log(record.releases[i]['release-group'].title.toLowerCase());
                                    album = record.releases[i]['release-group'].title;
                                }
                            }
                        }
                        else album = record.releases[0]['release-group'].title;
                        mbid = record.releases[0].id;
                    }
                    // else album = "Unknow Album";
                    ...
                    recordId = record.id;
                    newImport(artist, album, title, id++, mbid, genre, length, recordId);
                    totalSearch++;
                    nb++;                     
